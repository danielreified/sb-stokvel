...
export function createStokvelRouter(
  ...
  router.get('/:stokvelId', authMiddleware, async (c) => {
    const stokvel = await stokvelRepo.findById(toStokvelId(c.req.param('stokvelId')));
    if (!stokvel) return c.json({ error: 'not_found' }, 404);
    return c.json(stokvel);
  });

  router.get('/:stokvelId/members', authMiddleware, async (c) => {
    const members = await stokvelRepo.findMembers(toStokvelId(c.req.param('stokvelId')));
    return c.json(members);
  });

  router.get('/:stokvelId/contributions', authMiddleware, async (c) => {
    const contributions = await stokvelRepo.findContributions(
      toStokvelId(c.req.param('stokvelId')),
      {
  ...
  router.get('/:stokvelId/balance', authMiddleware, async (c) => {
    const balance = await stokvelRepo.findBalance(toStokvelId(c.req.param('stokvelId')));
  ...
  router.post('/:stokvelId/contributions', authMiddleware, async (c) => {
    ...
    await stokvelRepo.addContribution(contribution);
