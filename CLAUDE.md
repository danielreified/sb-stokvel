...
- **Max-staleness gate (the guardrail)**: if `now - lastSuccessfulCheckAt > MAX_STALENESS_MS`, the client refuses to render the authenticated tree regardless of stored `updateLevel`. It shows a "Couldn't verify app version — connect to a network and try again" screen with a retry button. This closes the attack where a network-positioned adversary black-holes only `/api/app/version-check` to keep a vulnerable old client running indefinitely.
- `MAX_STALENESS_MS` is **a configurable knob, not a constant** — lives in `apps/stokvel-app/src/config/version-guard.ts` (or read from a `VITE_MAX_STALENESS_MS` env var with a sensible default). The default is **24 hours** for the demo (banking-grade conservatism). For real-world deployment to SA stokvel users — many in connectivity-sparse rural areas with legitimate weekend off-grid time — **72 hours is probably more appropriate**, and per-deployment tuning may be needed. Document this tradeoff in the README: stricter = better security against the network-block attack; looser = better UX for low-connectivity users. Pick the threat model that matches the deployment, don't hardcode the banking number for everyone.
...
- **`forceUpdate()` deliberately differs from `signOut()`**: `signOut()` preserves the Workbox precache so `/login` loads on a flaky connection; `forceUpdate()` clears *all* caches including the precache. This is intentional — a forced update means "you cannot continue on this version, period," so the user must regain connectivity to download a fresh app shell. The two flows are not symmetrical; future maintainers reading both should not "helpfully" align them.
...
- **Security headers** middleware (third, before any routes):
  - **Strict CSP** (dev may relax for HMR; prod is strict):
    - `default-src 'self'`
    - `script-src 'self' 'strict-dynamic' 'sha256-<vite-bootstrap-hash>'` — **NO `'unsafe-inline'`**. Vite's inline bootstrap script is hashed at build time and the hash injected into the CSP via a Vite plugin (e.g. `vite-plugin-csp-guard` or a small custom plugin). `'strict-dynamic'` lets the bootstrap dynamically load further hashed scripts without each needing its own hash entry. **Hashes chosen over per-request nonces because the PWA is statically hosted** — there's no SSR layer to inject a fresh nonce per response. If SSR is ever introduced (BFF rendering `index.html` on every request), switch to nonces; they're easier to manage when third-party scripts (Sentry, analytics) get added.
...
- **CSP report sink**: `POST /api/csp-report` accepts a CSP violation report (`application/csp-report` or `application/reports+json`), logs it via the structured logger, returns 204. Unauthenticated — but **rate-limited per-IP** (generous bucket: 50/min/IP) to prevent an attacker flooding the endpoint with garbage reports to balloon logs and disk. Reports beyond the bucket are dropped silently (no body, no log entry).
...
> **Note for anyone reading this file from a fresh clone**: the full test bar and the full README/deliverables list live in the project's task tracker (Tasks #15 and #16), not in this file — the original short-list versions outgrew this file during spec rounds. Once the build lands, Task #16's final pass regenerates canonical *Testing* and *Deliverables* sections back into CLAUDE.md so this file is again self-sufficient. Until then, treat those two sections as deliberately deferred, not missing.
